...
export interface SelectProps extends SelectHTMLAttributes<HTMLSelectElement> {
   ...
   * `Select` sitting in the same row as a `Button`. An inline style, not
   * a Tailwind class: Tailwind v4 doesn't guarantee a later class in the
   * same string wins over `h-[54px]` above based on source order, while
   * an inline style always wins regardless (matches apps/dashboard's
   * Select).
   */
  compact?: boolean;
}

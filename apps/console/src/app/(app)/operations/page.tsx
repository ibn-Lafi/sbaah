...
export default function OperationsPage(){
 ...
 return <ConsoleShell title="تشغيل المنصة">
  <div className="mb-6"><h1 className="text-2xl font-semibold text-text-primary">مراقبة تشغيل المنصة</h1><p className="mt-1 text-sm text-text-secondary">نظرة مركزية على استخدام سبعة ونشاط الخدمات عبر جميع الحسابات.</p></div>
  {!data&&!error?<LoadingState/>:error?<Card className="p-5 text-sm text-danger">تعذّر تحميل بيانات التشغيل.</Card>:data&&<>
   <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{cards.map(([l,v])=><Card key={String(l)} className="p-5"><p className="text-sm text-text-secondary">{l}</p><p className="mt-3 text-3xl font-semibold text-text-primary">{Number(v).toLocaleString('ar-SA')}</p></Card>)}</div>
   <div className="mt-6 grid gap-4 lg:grid-cols-2">
    <Card className="p-5"><h2 className="font-semibold text-text-primary">النشاط خلال 30 يومًا</h2><div className="mt-4 grid grid-cols-3 gap-3">{[['عقارات جديدة',data.last_30_days.properties],['مشاريع جديدة',data.last_30_days.projects],['عملاء CRM جدد',data.last_30_days.leads]].map(([l,v])=><div key={String(l)} className="rounded-xl bg-surface-subtle p-4"><p className="text-xs text-text-secondary">{l}</p><p className="mt-2 text-xl font-semibold text-text-primary">{Number(v).toLocaleString('ar-SA')}</p></div>)}</div></Card>

...
export default function FaqManagementPage(){
 ...
 async function move(index:number,direction:-1|1){
  ...
  const current=items[index], other=items[target];
  setError('');
  try{
   await updateFaq(accessToken,current.id,{order_index:other.order_index});
   await updateFaq(accessToken,other.id,{order_index:current.order_index});
   await load();
 ...
 return <ConsoleShell title="الأسئلة الشائعة">
  ...
  <div className="mt-6 max-w-[900px] space-y-3">{items===null?<LoadingState/>:items.map((item,index)=><Card key={item.id} className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="font-semibold">{item.question_ar}</p><p dir="ltr" className="mt-1 text-sm text-text-secondary">{item.question_en}</p><p className="mt-3 text-sm text-text-secondary">{item.answer_ar}</p></div><div className="flex gap-2"><Button onClick={()=>patch(item.id,{is_active:!item.is_active})}>{item.is_active?'إخفاء':'إظهار'}</Button><Button onClick={()=>move(index,-1)} disabled={index===0}>↑</Button><Button onClick={()=>move(index,1)} disabled={index===items.length-1}>↓</Button><Button onClick={()=>remove(item.id)}>حذف</Button></div></div></Card>)}</div>
 </ConsoleShell>
}

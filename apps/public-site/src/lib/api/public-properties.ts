...
export interface PublicPropertySearchFilters { city_id?:string; district_id?:string; property_type?:string; listing_type?:string; min_price?:number; max_price?:number; bedrooms?:number; page?:number; page_size?:number; scope?:'independent'|'project'|'all'; }
...
export async function listPublicProperties(filters:PublicPropertySearchFilters):Promise<PublicPropertyListResponse>{
  ...
  if(filters.bedrooms!==undefined)query.set('bedrooms',String(filters.bedrooms)); if(filters.page)query.set('page',String(filters.page)); if(filters.page_size)query.set('page_size',String(filters.page_size)); if(filters.scope)query.set('scope',filters.scope);

  ...
  return (
    <a href={href} className="block overflow-hidden rounded-xl border border-black/10 transition-shadow hover:border-tenant-secondary/35 hover:shadow-md">
      ...
      <div className="flex flex-col gap-1 p-4">
        <span className="text-xs font-medium text-tenant-primary">{property.listing_type ? getListingTypeLabel(locale, property.listing_type) : getPropertyTypeLabel(locale, property.property_type)}</span>
        ...
        <p className="mt-1 font-semibold text-tenant-secondary">{property.price != null ? formatPrice(locale, property.price) : null}</p>
      </div>
    </a>
  );
}

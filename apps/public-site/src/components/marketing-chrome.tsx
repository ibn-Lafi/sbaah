...
function BrandMark({ label, invert = false, className = 'h-6' }: { label: string; invert?: boolean; className?: string }) {
  return <img src={invert ? '/sbaah-lockup-white.svg' : '/sbaah-lockup-purple.svg'} alt={label} className={`block w-auto max-w-full object-contain ${className}`} />;
}
    ...
    <>
      <header className={`fixed inset-x-0 top-0 z-50 text-white transition-all duration-300 ${scrolled ? 'border-b border-white/10 bg-brand/95 shadow-lg backdrop-blur-md' : 'border-b border-transparent bg-transparent shadow-none backdrop-blur-none'}`}>
        <div className="mx-auto hidden h-16 max-w-6xl items-center justify-between px-6 lg:flex">
          <a href={homeHref} className="flex items-center"><BrandMark label={t.brand} invert className="h-9" /></a>
          ...
          <div className="flex items-center gap-2.5 text-sm font-medium"><a href={otherLocaleHref} className="flex h-9 w-9 items-center justify-center rounded-xl text-white/90 hover:bg-white/10 hover:text-white" aria-label={t.nav.languageSwitch}><GlobeIcon className="h-4 w-4" /></a><ThemeToggle labels={t.nav} className="h-9 w-9 rounded-xl" /><a href={`${dashboardUrl}/login`} className="inline-flex h-9 items-center rounded-xl border border-white/30 px-4 text-white transition-colors hover:bg-white/10">{t.nav.login}</a><a href={`${dashboardUrl}/register`} className="text-brand inline-flex h-9 items-center rounded-xl bg-white px-4 font-semibold transition-colors hover:bg-white/90">{t.nav.cta}</a></div>
        </div>
        <div dir="ltr" className="flex h-16 w-full items-center justify-between px-4 lg:hidden"><div className="flex min-w-0 items-center gap-2"><a href={`${dashboardUrl}/register`} dir={locale === 'ar' ? 'rtl' : 'ltr'} className="text-brand flex h-7 flex-none items-center whitespace-nowrap rounded-[9px] bg-white px-3 text-xs font-semibold leading-none shadow-sm transition-colors hover:bg-white/90">{t.nav.cta}</a><a href={`${dashboardUrl}/login`} dir={locale === 'ar' ? 'rtl' : 'ltr'} className="flex h-7 flex-none items-center whitespace-nowrap rounded-[9px] border border-white/35 px-3 text-xs font-semibold leading-none text-white transition-colors hover:bg-white/10">{t.nav.login}</a></div><a href={homeHref} aria-label={t.brand} className="flex h-9 flex-none items-center justify-center"><BrandMark label={t.brand} invert className="h-8 max-[359px]:h-7" /></a></div>
      </header>
      <div className="h-16" />
      <main>{children}</main>
      <div className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-4 z-[60] flex flex-col items-center gap-2 lg:hidden"><div className={`flex flex-col items-center gap-2 transition-all duration-200 ${quickControlsOpen ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-3 opacity-0'}`} aria-hidden={!quickControlsOpen}><a href={otherLocaleHref} aria-label={t.nav.languageSwitch} title={t.nav.languageSwitch} className="text-brand flex h-11 w-11 items-center justify-center rounded-full border border-border-subtle bg-surface-card shadow-lg transition-transform hover:scale-105"><GlobeIcon className="h-5 w-5" /></a><div className="flex h-11 w-11 items-center justify-center rounded-full border border-border-subtle bg-surface-card text-text-primary shadow-lg"><ThemeToggle labels={t.nav} className="h-11 w-11 rounded-full bg-transparent text-current hover:bg-black/5" /></div></div><button type="button" onClick={() => setQuickControlsOpen((value) => !value)} aria-label={quickControlsOpen ? t.nav.menuClose : t.nav.menuOpen} aria-expanded={quickControlsOpen} className="bg-brand flex h-12 w-12 items-center justify-center rounded-full text-white shadow-xl ring-1 ring-white/20 transition-transform active:scale-95">{quickControlsOpen ? <CloseIcon className="h-5 w-5" /> : <QuickControlsIcon className="h-5 w-5" />}</button></div>
          ...
          <div className="relative flex flex-col items-center px-6 py-12 text-center sm:px-10 sm:py-16">
            ...
            <div className="mt-12">
              ...
              <nav className="mt-5 flex flex-wrap items-center justify-center gap-x-7 gap-y-4 text-sm text-white/80 sm:text-base">
                ...
                <a href={localizedHref('/support')} className="hover:text-white">{locale === 'ar' ? 'مركز الدعم' : 'Support Center'}</a>
              </nav>
            </div>

            <div className="mt-14 flex flex-wrap items-center justify-center gap-3 text-xs sm:text-sm">
              <a href={localizedHref('/privacy')} className="rounded-full border border-white/55 px-5 py-2.5 text-white/85 hover:bg-white/10">{locale === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy'}</a>
              <a href={localizedHref('/terms')} className="rounded-full border border-white/55 px-5 py-2.5 text-white/85 hover:bg-white/10">{locale === 'ar' ? 'الشروط والأحكام' : 'Terms & Conditions'}</a>
            </div>

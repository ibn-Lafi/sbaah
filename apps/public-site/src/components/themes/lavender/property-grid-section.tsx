...
export async function PropertyGridSection({locale,config}:PropertyGridSectionProps){
 ...
 return <LavenderSection className="bg-[#f4f1ea]"><LavenderHeading eyebrow={locale==='ar'?'مختارات عقارية':'Selected properties'} title={title} action={<a href={href} className="hidden text-sm font-semibold text-black underline-offset-4 hover:underline sm:block">{locale==='ar'?'عرض الكل':'See all'}</a>}/>

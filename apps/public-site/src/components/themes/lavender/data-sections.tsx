...
const More=({locale,href}:{locale:Locale;href:string})=><a href={localizedPath(locale,href)} className="hidden border-b border-black/40 pb-1 text-sm text-black sm:block">{copy[locale].all}</a>;
...
export async function LavenderProjects({locale,config}:{locale:Locale;config:ProjectsShowcaseSectionConfig}){
 ...
 return <LavenderSection className="bg-[#171713] text-white"><div className="mb-10 flex items-end justify-between border-b border-white/20 pb-5 sm:mb-14"><h2 className="text-3xl font-medium sm:text-5xl">{config.title_ar||copy[locale].projects}</h2><a href={localizedPath(locale,'/projects')} className="hidden border-b border-white/50 pb-1 text-sm sm:block">{copy[locale].all}</a></div><div className="grid gap-5 lg:grid-cols-3">{details.map((p,i)=><LavenderProject key={p.id} project={p} city={city.get(p.city_id)} locale={locale} featured={i===0}/>)}</div></LavenderSection>;
}

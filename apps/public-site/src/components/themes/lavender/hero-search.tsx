...
const fieldClass =
  'h-14 w-full border-0 bg-transparent px-0 text-sm text-[#171a17] outline-none [color-scheme:light]';
...
export function LavenderHeroSearch({
  ...
  return (
    <div className="w-full bg-[#f7f5ef] text-[#171a17] shadow-[0_22px_70px_rgba(0,0,0,.2)]">
      {scopes.length > 1 && (
        <div
        ...
        >
          {scopes.map((value) => (
            <button
              ...
              className={`relative min-h-14 flex-1 px-5 text-sm font-semibold transition-colors sm:flex-none sm:px-9 ${scope === value ? 'after:bg-tenant-primary text-[#171a17] after:absolute after:inset-x-5 after:bottom-0 after:h-0.5 sm:after:inset-x-9' : 'text-black/45 hover:text-black'}`}
            >
              {t[value]}
            </button>
          ))}
      ...
      <form method="get" action={action} className="grid lg:grid-cols-[1fr_1fr_1fr_auto]">
        {scope === 'properties' && (
          <label className="border-b border-black/10 px-5 py-3 lg:border-b-0 lg:border-e">
        ...
        <label
          className={`border-b border-black/10 px-5 py-3 lg:border-b-0 lg:border-e ${scope === 'projects' ? 'lg:col-span-3' : ''}`}
        >
        ...
        <button
          type="submit"
          className="bg-tenant-primary min-h-20 px-8 text-sm font-semibold text-white transition-opacity hover:opacity-90 lg:min-w-48"
        >

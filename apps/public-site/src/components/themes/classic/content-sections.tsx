...
export function StatsSection({ locale, config }: { locale: Locale; config: StatsSectionConfig }) {
  ...
  return <ClassicSection className={classicToneClass(config.tone ?? 'soft')}><ClassicSectionHeading title={config.title_ar || copy[locale].stats} align={config.heading_align ?? 'center'} />{items.length ? <div className={`grid grid-cols-2 gap-3 ${classicColumnsClass(config.columns ?? 4)}`}>{items.map((item, index) => <div key={index} className="rounded-2xl border border-black/10 bg-white p-5 text-center shadow-sm sm:p-6"><strong className="block text-2xl font-bold text-tenant-primary sm:text-3xl">{item.value}</strong><span className="mt-2 block text-sm leading-6 text-black/55">{item.label}</span></div>)}</div> : <ClassicEmptyState>{copy[locale].empty}</ClassicEmptyState>}</ClassicSection>;
}
...
export function FaqSection({ locale, config }: { locale: Locale; config: FaqSectionConfig }) {
  ...
  return <ClassicSection className={classicToneClass(config.tone ?? 'soft')}><div className="mx-auto max-w-4xl"><ClassicSectionHeading title={config.title_ar || copy[locale].faq} align={config.heading_align ?? 'center'} />{items.length ? <div className="overflow-hidden rounded-2xl border border-black/10 bg-white">{items.map((item, index) => <details key={index} className="group border-b border-black/10 last:border-b-0"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-5 font-semibold sm:px-6"><span>{item.question}</span><span className="text-xl font-normal text-tenant-primary transition-transform group-open:rotate-45" aria-hidden="true">+</span></summary><p className="px-5 pb-5 text-sm leading-7 text-black/60 sm:px-6">{item.answer}</p></details>)}</div> : <ClassicEmptyState>{copy[locale].empty}</ClassicEmptyState>}</div></ClassicSection>;
}

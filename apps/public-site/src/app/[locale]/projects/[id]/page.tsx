...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  const leadBody = typeof leadSection?.config.body_ar === 'string' ? leadSection.config.body_ar : '';
  if(isLavender) return <>{detailVisible&&<LavenderProjectDetail locale={locale} data={data}/>} {leadSection&&<section className="bg-[#f4f1ea] px-4 py-12 sm:px-6 sm:py-20"><div className="mx-auto grid max-w-7xl gap-7 lg:grid-cols-[.7fr_1.3fr]"><div><p className="mb-2 text-xs text-black/45">{locale==='ar'?'تواصل معنا':'Get in touch'}</p><h2 className="text-3xl font-medium sm:text-5xl">{String(leadSection.config.title_ar??(locale==='ar'?'سجل اهتمامك بالمشروع':'Register your interest'))}</h2>{leadBody&&<p className="mt-3 max-w-md text-sm leading-7 text-black/55">{leadBody}</p>}</div><InquiryForm locale={locale} tenantId={site.tenant.id} projectId={project.id} variant="lavender"/></div></section>}</>;
  const title=pickLocalized(locale,project.name_ar,project.name_en);

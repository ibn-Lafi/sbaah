...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description =
    pickLocalized(locale, project.description_ar ?? '', project.description_en ?? null) ||
    undefined;
...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  const project = data.project;
  if (id !== project.slug) permanentRedirect(localizedPath(locale, `/projects/${project.slug}`));
  ...
  const detailSection = site.sections.find((s) => s.type === 'project_detail');
  ...
  const showcase = [...by('general'), ...by('exterior')]
    .filter((m) => m.id !== primary?.id)
    .slice(0, 6);
  const sections = [
    {
      key: 'master_plan',
      title: locale === 'ar' ? 'مخطط المشروع' : 'Master plan',
      items: by('master_plan'),
    },
    {
      key: 'unit_plans',
      title: locale === 'ar' ? 'مخططات الوحدات' : 'Unit plans',
      items: by('unit_plans'),
    },
    {
      key: 'interior',
      title: locale === 'ar' ? 'التصاميم والمساحات الداخلية' : 'Interiors',
      items: by('interior'),
    },
    {
      key: 'amenities',
      title: locale === 'ar' ? 'المرافق والخدمات' : 'Amenities & services',
      items: by('amenities'),
    },
    {
      key: 'location',
      title: locale === 'ar' ? 'الموقع والمحيط' : 'Location & surroundings',
      items: by('location'),
    },
    {
      key: 'construction',
      title: locale === 'ar' ? 'تقدم المشروع' : 'Construction progress',
      items: by('construction'),
    },
  ];
  const completion =
    typeof project.completion_percentage === 'number' ? project.completion_percentage : null;
  return (
    <main className="pb-16">
      <section className="relative min-h-[420px] overflow-hidden bg-black md:min-h-[560px]">
        {primary ? (
          <img
            src={primary.url}
            alt={pickLocalized(locale, primary.alt_ar ?? title, primary.alt_en)}
        ...
        <div className="relative mx-auto flex min-h-[420px] max-w-6xl items-end px-6 pb-10 text-white md:min-h-[560px] md:pb-14">
          <div className="max-w-3xl">
            ...
            {description && (
              <p className="mt-4 line-clamp-3 max-w-2xl text-sm leading-7 text-white/85 md:text-base">
                {description}
              </p>
            )}
          </div>
      ...
      <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
              {showcase.map((m, i) => (
                <img
                  key={m.id}
                  src={m.url}
                  alt={pickLocalized(locale, m.alt_ar ?? title, m.alt_en)}
                  loading="lazy"
                  className={`w-full rounded-2xl object-cover ${i === 0 ? 'col-span-2 aspect-[2/1] md:col-span-1 md:aspect-[4/3]' : 'aspect-[4/3]'}`}
                />
              ))}
            </div>
          </section>
        )}
        <section className="grid gap-5 border-b border-black/10 py-10 md:grid-cols-[1fr_auto]">
          <div>
            <h2 className="text-2xl font-semibold">
              {locale === 'ar' ? 'عن المشروع' : 'About the project'}
            </h2>
            {description && (
              <p className="mt-4 max-w-3xl whitespace-pre-line leading-8 text-black/70">
                {description}
              </p>
            )}
          </div>
          <dl className="grid min-w-[260px] grid-cols-2 gap-4 text-sm">
            {completion != null && (
              <div>
                <dt className="text-black/50">{locale === 'ar' ? 'نسبة الإنجاز' : 'Completion'}</dt>
                <dd className="mt-1 font-semibold">{completion}%</dd>
              </div>
            )}
            {project.planned_units_count != null && (
              <div>
                <dt className="text-black/50">
                  {locale === 'ar' ? 'عدد الوحدات' : 'Planned units'}
                </dt>
                <dd className="mt-1 font-semibold">{String(project.planned_units_count)}</dd>
              </div>
            )}
            {project.expected_completion_date != null && (
              <div className="col-span-2">
                ...
                <dd className="mt-1 font-semibold">{String(project.expected_completion_date)}</dd>
              </div>
            )}
          </dl>
        </section>
        {data.unit_types.length > 0 && (
          <section className="border-b border-black/10 py-10">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {data.unit_types.map((type) => (
                <article key={type.id} className="rounded-2xl border border-black/10 p-5">
        ...
        {sections.map(
          (section) =>
            section.items.length > 0 && (
              <section key={section.key} className="border-b border-black/10 py-10">
                <div className="mb-5 flex items-end justify-between gap-4">
                  <h2 className="text-2xl font-semibold">{section.title}</h2>
                  <span className="text-sm text-black/50">{section.items.length}</span>
                </div>
                <div
                  className={
                    section.key === 'master_plan'
                      ? 'grid gap-4'
                ...
                >
                  {section.items.map((m) => (
                    <img
                      key={m.id}
                      src={m.url}
                      alt={pickLocalized(locale, m.alt_ar ?? section.title, m.alt_en)}
                      loading="lazy"
                      className={
                        section.key === 'master_plan'
                          ? 'max-h-[720px] w-full rounded-2xl bg-black/[.03] object-contain'
        ...
        {leadSection && (
          <section className="border-t border-black/10 py-10">
            ...
            <InquiryForm locale={locale} tenantId={site.tenant.id} projectId={project.id} />
          </section>
        )}
        <section className="py-10">
          ...
          {data.units.length === 0 ? (
            <p className="mt-3 text-black/60">
              {locale === 'ar' ? 'لا توجد وحدات متاحة حاليًا' : 'No units currently available'}
            </p>
          ) : (
            <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {data.units.map((unit) => (
                <article key={unit.id} className="rounded-2xl border border-black/10 p-5">
                  <h3 className="font-semibold">
                    {unit.name_ar ||
                      `${locale === 'ar' ? 'وحدة' : 'Unit'} ${unit.unit_number ?? ''}`}
                  </h3>
                  {unit.area_sqm != null && (
                    <p className="mt-2 text-sm text-black/60">{unit.area_sqm} m²</p>
                  )}
                  {unit.price != null && (

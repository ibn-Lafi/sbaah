...
/** "من نحن" page (متجر الثيمات follow-up, migration 0024) — every section here is themed (no data-driven anchor, unlike /properties or /projects). */
export default async function AboutPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  const locale = isLocale(rawLocale) ? rawLocale : DEFAULT_LOCALE;

  const site = await getTenantSitePage('about');
  if (!site) return null;

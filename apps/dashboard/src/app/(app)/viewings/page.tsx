...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([listViewings(accessToken), listTasks(accessToken), listLeads(accessToken, {})]).then(([v, t, l]) => {
      ...
      setTasks(t.tasks);
      setLeads(l.leads);
      void Promise.all([listInstallments(accessToken), listLeaseContracts(accessToken), listAssets(accessToken,{page_size:50})])
        .then(([i,c,a])=>{ if(active){setInstallments(i.installments);setContracts(c.contracts);setAssets(a.assets);} })
        .catch(()=>{});
    });
    return () => { active = false; };
  }, [accessToken]);

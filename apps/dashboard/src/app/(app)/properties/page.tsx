...
export default function PropertiesPage() {
  ...
  return <AppShell title="العقارات" orgName={me.tenant.name_ar} accountType={me.tenant.account_type}>
    ...
    <Card className="overflow-hidden">{assets === null ? <TableSkeleton columns={3} /> : assets.length === 0 ? <p className="p-8 text-center text-text-secondary">لا توجد عقارات مطابقة</p> : <><div className="divide-y divide-border-subtle md:hidden">{assets.map(asset => <button key={asset.id} onClick={() => router.push(`/properties/${asset.id}`)} className="flex w-full items-center justify-between gap-3 p-4 text-start"><div className="min-w-0"><p className="truncate font-semibold text-text-primary">{asset.name_ar}</p><p className="mt-1 text-xs text-text-secondary">{assetTypeLabels[asset.asset_type]}{asset.reference_number ? ` · ${asset.reference_number}` : ''}</p></div><span className="shrink-0 text-xs font-medium text-brand">{statusLabels[asset.physical_status]}</span></button>)}</div><table className="hidden w-full table-fixed text-sm md:table"><thead className="bg-surface-header text-right text-text-secondary"><tr><th className="px-4 py-3 font-medium">العقار</th><th className="px-4 py-3 font-medium">النوع</th><th className="px-4 py-3 font-medium">الحالة</th></tr></thead><tbody>{assets.map(asset => <tr key={asset.id} className="border-border-subtle border-t"><td className="px-4 py-3"><button onClick={() => router.push(`/properties/${asset.id}`)} className="font-medium hover:text-brand">{asset.name_ar}</button></td><td className="px-4 py-3 text-text-secondary">{assetTypeLabels[asset.asset_type]}</td><td className="px-4 py-3">{statusLabels[asset.physical_status]}</td></tr>)}</tbody></table></>}</Card>
  </AppShell>;
}

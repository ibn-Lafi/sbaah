...
export default function ListingPage({params}:{params:Promise<{id:string}>}){
 const {id}=use(params); const {me,accessToken}=useCurrentUser();
 ...
 {!data?<FormPageSkeleton fields={6} extraCards={1}/>:<div className="mx-auto flex max-w-[820px] flex-col gap-5">
  ...
  <Card className="p-6"><h2 className="mb-4 font-semibold">العقارات المرتبطة</h2>{data.listing_assets.map(x=><a key={x.asset_id} href={'/properties/'+x.asset_id} className="block py-1 text-sm font-medium text-brand hover:underline">{x.assets?.name_ar??x.asset_id}</a>)}</Card>
  ...
  {me.user.role!=='agent'&&data.publication_status!=='archived'&&<div className="flex justify-end"><Button variant="secondary" onClick={async()=>{if(!confirm('هل تريد أرشفة العرض؟'))return;await archiveListing(accessToken,id);window.location.href='/properties'}}>أرشفة العرض</Button></div>}
 </div>}
 </AppShell>
}
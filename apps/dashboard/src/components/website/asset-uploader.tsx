  ...
  return <div className="flex min-w-0 flex-col gap-2">
    ...
    <div className="rounded-xl border border-dashed border-border-default bg-surface-card p-3 sm:p-4">
      {currentUrl?<div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div className="grid w-full grid-cols-2 gap-2 sm:w-auto"><Button type="button" variant="secondary" disabled={loading} onClick={()=>ref.current?.click()} className="h-10 min-w-28 justify-center gap-2"><UploadIcon/><span>{t.assetUploader.replace}</span></Button><Button type="button" variant="secondary" disabled={loading} onClick={()=>void onRemove()} className="h-10 min-w-28 justify-center gap-2"><TrashIcon/><span>{t.assetUploader.remove}</span></Button></div>

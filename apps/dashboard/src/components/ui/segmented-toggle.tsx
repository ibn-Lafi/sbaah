...
export function SegmentedToggle<T extends string>({ value, onChange, options, className = '' }: SegmentedToggleProps<T>) {
  return (
    <div className={`bg-surface-subtle-3 flex w-full gap-1 rounded-full p-1 ${className} ${className.includes('settings-tabs') ? 'max-sm:w-[calc(100vw-28px)] max-sm:max-w-none max-sm:justify-start max-sm:gap-1 max-sm:overflow-x-auto max-sm:rounded-[18px] max-sm:p-1 max-sm:[scrollbar-width:none] max-sm:[&::-webkit-scrollbar]:hidden' : ''}`}>
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          onClick={() => onChange(option.value)}
          className={`min-h-10 flex-1 rounded-full px-2 text-[13px] font-semibold leading-tight transition-colors ${className.includes('settings-tabs') ? 'max-sm:h-10 max-sm:min-h-10 max-sm:flex-none max-sm:whitespace-nowrap max-sm:px-3.5 max-sm:py-0 max-sm:text-[11px] max-sm:leading-none' : 'h-10'} ${

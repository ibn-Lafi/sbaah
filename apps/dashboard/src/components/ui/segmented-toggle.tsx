...
export function SegmentedToggle<T extends string>({ value, onChange, options, className = '' }: SegmentedToggleProps<T>) {
  return (
    <div className={`bg-surface-subtle-3 flex w-full gap-1 rounded-full p-1 ${className} ${className.includes('settings-tabs') ? 'max-sm:grid max-sm:grid-cols-2 max-sm:gap-1.5 max-sm:rounded-[22px] max-sm:p-1.5' : ''}`}>
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          onClick={() => onChange(option.value)}
          className={`min-h-10 flex-1 rounded-full px-2 text-[13px] font-semibold leading-tight transition-colors ${className.includes('settings-tabs') ? 'max-sm:min-h-11 max-sm:h-auto max-sm:px-3 max-sm:py-2.5 max-sm:text-[11px] max-sm:leading-4' : 'h-10'} ${

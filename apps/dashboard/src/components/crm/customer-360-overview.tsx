...
const AssetLink=({asset}:{asset?:{id:string;name_ar:string;unit_number:string|null}|null})=>asset?<a href={`/properties/${asset.id}`} className="font-medium text-brand hover:underline">{asset.name_ar}{asset.unit_number?` · وحدة ${asset.unit_number}`:''}</a>:<span className="text-text-secondary">العقار غير متاح</span>;
...
export function Customer360Overview({lead,data,tab='overview',accessToken,onChanged}:{lead:LeadWithNotes;data:Customer360Snapshot;tab?:CustomerDetailTab;accessToken?:string;onChanged?:()=>Promise<void>|void}) {
  ...
  if(tab==='purchase') { const purchasedAssets=data.purchased_assets; return <div className="grid gap-3 md:grid-cols-3">
    <Card className="p-4 md:p-5 md:col-span-3"><h2 className="mb-3 font-semibold">العقارات المشتراة</h2>{purchasedAssets.length===0?<p className="text-sm text-text-secondary">لا توجد عقارات مشتراة مكتملة مرتبطة بهذا العميل.</p>:<div className="flex flex-wrap gap-2">{purchasedAssets.map((asset)=><div key={asset.id} className="rounded-input border border-border-subtle px-3 py-2"><AssetLink asset={asset}/></div>)}</div>}</Card>

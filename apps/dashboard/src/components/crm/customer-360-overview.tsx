...
export function Customer360Overview({lead,data,tab='overview'}:{lead:LeadWithNotes;data:Customer360Snapshot;tab?:CustomerDetailTab}) {
  ...
  if(tab==='rent') { const rentedAssets=data.contracts.filter(c=>c.customer_roles?.includes('lessee')&&!['cancelled','terminated'].includes(c.status)).flatMap(c=>c.lease_contract_assets??[]); return <div className="grid gap-3 lg:grid-cols-2">
    <Card className="p-4 md:p-5 lg:col-span-2"><h2 className="mb-3 font-semibold">العقارات المستأجرة</h2>{rentedAssets.length===0?<p className="text-sm text-text-secondary">لا توجد عقارات مستأجرة فعالة مرتبطة بهذا العميل.</p>:<div className="flex flex-wrap gap-2">{rentedAssets.map((x,index)=><div key={x.asset_id+index} className="rounded-input border border-border-subtle px-3 py-2"><AssetLink asset={x.assets}/></div>)}</div>}</Card>
  ...
  if(tab==='purchase') { const purchasedAssets=data.deals.filter(d=>d.status==='won').flatMap(d=>d.deal_assets??[]); return <div className="grid gap-3 md:grid-cols-3">
    <Card className="p-4 md:p-5 md:col-span-3"><h2 className="mb-3 font-semibold">العقارات المشتراة</h2>{purchasedAssets.length===0?<p className="text-sm text-text-secondary">لا توجد عقارات مشتراة مكتملة مرتبطة بهذا العميل.</p>:<div className="flex flex-wrap gap-2">{purchasedAssets.map((x,index)=><div key={x.asset_id+index} className="rounded-input border border-border-subtle px-3 py-2"><AssetLink asset={x.assets}/></div>)}</div>}</Card>

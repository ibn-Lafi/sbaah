...
export function CustomerActivityTimeline({lead,data}:{lead:LeadWithNotes;data:Customer360Snapshot}){
  ...
  return <Card className="p-4 md:p-5">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-text-primary">سجل العميل</h2><p className="mt-1 text-xs text-text-secondary">تسلسل موحّد لما حدث مع العميل داخل المنصة.</p></div><span className="text-xs text-text-secondary">{items.length} حدث</span></div>
    <div className="-mx-1 mb-5 flex gap-1 overflow-x-auto px-1 pb-1">{(Object.keys(labels) as Filter[]).map(key=><button key={key} type="button" onClick={()=>{setFilter(key);setExpanded(false)}} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs transition-colors ${filter===key?'border-brand bg-brand text-white':'border-border-default bg-surface-card text-text-secondary hover:border-brand'}`}>{labels[key]}</button>)}</div>
    {shown.length===0?<p className="text-sm text-text-secondary">لا توجد أحداث ضمن هذا التصنيف.</p>:<div className="flex flex-col">{shown.map((item,index)=><div key={item.id} className="relative flex gap-3 pb-5 last:pb-0"><div className="flex w-3 flex-col items-center"><span className="mt-1.5 h-2.5 w-2.5 rounded-full bg-brand"/>{index<shown.length-1&&<span className="mt-1 w-px flex-1 bg-border-subtle"/>}</div><div className="min-w-0"><p className="text-xs text-text-secondary">{item.label}</p><p className="mt-0.5 text-sm font-medium text-text-primary">{item.title}</p><p className="mt-1 text-xs text-text-secondary">{formatRelativeTime(item.at)}</p></div></div>)}</div>}

 ...
 */
const PINNED_KEYS = ['/', 'properties', 'clients'];
          ...
          {pinnedItems.map(({ key, href, label, icon: ItemIcon, activeHrefs }) => {
            ...
            return (
              <Link
                key={key}
                href={href}
                className={`flex min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-[14px] px-1.5 py-1 text-[10px] leading-none ${
                  active

...
function toFormState(property: PropertyWithMedia): FormState {
  return {
    ...
    land_area: String((property as any).land_area ?? ''), built_area: String((property as any).built_area ?? ''), street_width: String((property as any).street_width ?? ''), property_age: String((property as any).property_age ?? ''), floor_number: String((property as any).floor_number ?? ''), floors_count: String((property as any).floors_count ?? ''), parking_count: String((property as any).parking_count ?? ''), elevators_count: String((property as any).elevators_count ?? ''), reference_number: String((property as any).reference_number ?? ''), advertisement_license_number: String((property as any).advertisement_license_number ?? ''), advertisement_license_expires_at: (property as any).advertisement_license_expires_at ? String((property as any).advertisement_license_expires_at).slice(0, 16) : '', advertiser_name: String((property as any).advertiser_name ?? ''),
  };
}

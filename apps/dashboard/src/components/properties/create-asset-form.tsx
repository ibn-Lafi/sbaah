...
export function CreateAssetForm({accessToken,onCreated,projectId,parentAssetId,parentAssetName}:{accessToken:string;onCreated:(id:string)=>void;projectId?:string;parentAssetId?:string;parentAssetName?:string;}){
 ...
 const isLand=['land','plot'].includes(form.type);
 const isCommercial=['shop','office','warehouse'].includes(form.type);
 ...
 return <form onSubmit={submit} className="flex flex-col gap-5">
  ...
  {!parentAssetId&&projectId&&<div className="rounded-lg bg-surface-subtle-3 p-3 text-sm">هذا العقار تابع للمشروع.<p className="mt-1 text-xs text-text-secondary">الموقع الجغرافي يُورث من المشروع، ووسائط العقار تُدار من معرض المشروع.</p></div>}
  ...
  {step===1&&<div className="space-y-5">
   <div className="grid gap-3 sm:grid-cols-2">
    ...
    <label className="flex h-11 items-center rounded-lg border border-border-default bg-surface px-3"><span className="text-sm text-text-secondary">المساحة</span><input inputMode="decimal" value={form.area} onChange={e=>set('area',e.target.value)} type="number" min="0.01" step="0.01" className="min-w-0 flex-1 bg-transparent px-3 text-center text-sm font-semibold outline-none" placeholder="0"/><span className="text-xs text-text-secondary">م²</span></label>
    ...
    {!isLand&&<Counter label="المواقف" value={form.parkingCount} onChange={v=>set('parkingCount',v)}/>}
    {hasBuilding&&!isCommercial&&<Counter label="المصاعد" value={form.elevatorsCount} onChange={v=>set('elevatorsCount',v)}/>}
    {!parentAssetId&&<><Input value={form.landArea} onChange={e=>set('landArea',e.target.value)} type="number" min="0.01" step="0.01" placeholder="مساحة الأرض م²"/>{hasBuilding&&<Input value={form.builtArea} onChange={e=>set('builtArea',e.target.value)} type="number" min="0.01" step="0.01" placeholder="مساحة البناء م²"/>}{!inheritsLocation&&<Input value={form.streetWidth} onChange={e=>set('streetWidth',e.target.value)} type="number" min="0.01" step="0.01" placeholder="عرض الشارع م"/>} </>}
    {hasRooms&&<div className="sm:col-span-2"><p className="mb-2 text-xs text-text-secondary">التأثيث</p><div className="grid grid-cols-3 gap-2">{([['unfurnished','غير مفروش'],['semi_furnished','شبه مفروش'],['furnished','مفروش']] as const).map(([value,label])=><button key={value} type="button" onClick={()=>set('furnishing',form.furnishing===value?'':value)} className={`rounded-lg border px-2 py-2 text-xs font-medium ${form.furnishing===value?'border-brand bg-brand/5 text-brand':'border-border-default'}`}>{label}</button>)}</div></div>}
    <Input value={form.description} onChange={e=>set('description',e.target.value)} placeholder="وصف مختصر (اختياري)"/>
   </div>
   <div className="border-t border-border-subtle pt-4"><button type="button" onClick={()=>setShowAdvanced(v=>!v)} className="text-sm font-medium text-brand">{showAdvanced?'إخفاء المواصفات الإضافية':'مواصفات إضافية'}</button>{showAdvanced&&<div className="mt-3 grid gap-3 sm:grid-cols-2">{!parentAssetId&&hasBuilding&&<Counter label="عدد الأدوار" value={form.floorsCount} onChange={v=>set('floorsCount',v)}/>} {!parentAssetId&&<Input value={form.propertyAge} onChange={e=>set('propertyAge',e.target.value)} type="number" min="0" placeholder="عمر العقار (سنة)"/>}</div>}</div>
  ...
  {step===2&&<div className="space-y-4"><dl className="grid gap-4 rounded-xl border border-border-default p-4 text-sm sm:grid-cols-2"><div><dt className="text-text-secondary">{parentAssetId?'الوحدة':'العقار'}</dt><dd className="font-medium">{form.name}</dd></div><div><dt className="text-text-secondary">النوع</dt><dd>{assetTypeLabels[form.type]}</dd></div><div><dt className="text-text-secondary">المساحة</dt><dd>{form.area?`${form.area} م²`:'غير محددة'}</dd></div><div><dt className="text-text-secondary">الموقع</dt><dd>{inheritsLocation?'موروث من الأصل الرئيسي':form.location?'تم تحديده على الخريطة':'غير محدد'}</dd></div></dl>
   ...
   <p className="text-xs text-text-secondary">{projectId?'بعد الحفظ يمكنك إنشاء عرض بيع أو إيجار. الصور والفيديو تُدار من معرض المشروع.':'بعد الحفظ يمكنك إضافة الصور والفيديو وإنشاء عرض بيع أو إيجار. لا نطلب السعر أثناء إنشاء الأصل.'}</p>
  </div>}

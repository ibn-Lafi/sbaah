...
const updateSchema=z.object({
 question_ar:z.string().trim().min(2).max(300).optional(), answer_ar:z.string().trim().min(2).max(5000).optional(),
 question_en:z.string().trim().min(2).max(300).optional(), answer_en:z.string().trim().min(2).max(5000).optional(),
 order_index:z.number().int().nonnegative().optional(), is_active:z.boolean().optional(),
}).refine(input=>Object.values(input).some(value=>value!==undefined),{message:'يجب تحديد حقل واحد على الأقل للتحديث'});
const idSchema=z.string().uuid();
export const PATCH=withErrorHandling(async(request:NextRequest,{params}:{params:Promise<{id:string}>})=>{
 const {supabase}=await getPlatformAdminClient(request); const {id:rawId}=await params; const id=idSchema.parse(rawId); const input=updateSchema.parse(await request.json());
...
export const DELETE=withErrorHandling(async(request:NextRequest,{params}:{params:Promise<{id:string}>})=>{
 const {supabase}=await getPlatformAdminClient(request); const {id:rawId}=await params; const id=idSchema.parse(rawId);

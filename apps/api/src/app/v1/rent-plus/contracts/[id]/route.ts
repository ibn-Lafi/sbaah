...
export const GET=withErrorHandling<RouteContext>(async(request,{params})=>{
 ...
 const{data,error}=await supabase.from('lease_contracts').select('*, lease_contract_assets(asset_id,assets(*)), lease_contract_parties(party_id,role,parties(*)), lease_installments(*,lease_payment_allocations(amount,payment_id))').eq('id',id).maybeSingle();
 ...
 const installments=(data.lease_installments??[]).map((row:any)=>{const paid=(row.lease_payment_allocations??[]).reduce((s:number,a:{amount:number})=>s+Number(a.amount),0);return{...row,paid_amount:paid,remaining_amount:Math.max(0,Number(row.amount)-paid)}});

...
import { getAuthenticatedClient } from '@/lib/auth/get-authenticated-client'; import { getCallerContext } from '@/lib/auth/get-caller-context';
...
export const GET=withErrorHandling<RouteContext>(async(request,{params})=>{
 ...
 const{data:payments,error:paymentError}=await supabase.from('lease_payments').select('*,lease_payment_allocations(*)').eq('tenant_id',caller.tenantId).eq('contract_id',id).order('paid_at',{ascending:false});if(paymentError)throw new Error(`Failed to load contract payments: ${paymentError.message}`);
...
export const PATCH=withErrorHandling<RouteContext>(async(request,{params})=>{const{id}=await params;const{supabase}=getAuthenticatedClient(request);const caller=await getCallerContext(supabase);if(caller.role==='agent')throw new ApiError(403,'forbidden','لا يملك الوسيط صلاحية تعديل عقود الإيجار');const input=updateSchema.parse(await request.json());const payload={...input,...(input.status==='terminated'?{terminated_at:new Date().toISOString()}: {})};const{data,error}=await supabase.from('lease_contracts').update(payload).eq('id',id).eq('tenant_id',caller.tenantId).select().maybeSingle();if(error)throw new Error(`Failed to update lease contract: ${error.message}`);if(!data)throw new ApiError(404,'lease_contract_not_found','عقد الإيجار غير موجود');return okResponse({contract:data});});
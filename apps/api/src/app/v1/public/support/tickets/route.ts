...
const createSchema=z.object({requester_name:z.string().trim().min(2).max(120),requester_email:z.string().trim().toLowerCase().email().max(200),requester_phone:z.string().trim().max(30).optional().transform(v=>v||undefined),type:z.enum(['complaint','suggestion','support']),category:z.enum(['billing','technical','account','website','domain','other']),subject:z.string().trim().min(3).max(160),description:z.string().trim().min(10).max(5000)});
const trackSchema=z.object({ticket_number:z.string().trim().min(6).max(40),requester_email:z.string().trim().toLowerCase().email()});

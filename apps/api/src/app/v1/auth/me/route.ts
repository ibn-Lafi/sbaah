...
export const GET = withErrorHandling(async (request: NextRequest) => {
  ...
  const { data: tenant, error: tenantError } = await supabase
    .from('tenants')
    .select(
      'id, name_ar, name_en, account_type, subdomain, custom_domain, custom_domain_status, status, trial_ends_at, cr_number, tax_number, fal_license_number, social_instagram, social_tiktok, social_whatsapp, social_snapchat, social_phone',
    )

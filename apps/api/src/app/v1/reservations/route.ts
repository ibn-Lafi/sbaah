...
export const GET=withErrorHandling(async(request:NextRequest)=>{
 const {supabase}=getAuthenticatedClient(request); const {status,page,page_size}=querySchema.parse(Object.fromEntries(request.nextUrl.searchParams));
...
export const POST=withErrorHandling(async(request:NextRequest)=>{
 const {supabase}=getAuthenticatedClient(request); const input=reservationInputSchema.parse(await request.json()); const {asset_ids,...payload}=input;

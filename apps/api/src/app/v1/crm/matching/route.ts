...
export const GET=withErrorHandling(async(r:NextRequest)=>{
 ...
 const assetIds=[...new Set((listings??[]).flatMap((l:any)=>(l.listing_assets??[]).map((x:any)=>x.asset_id)))];
 ...
 const matches=(listings??[]).flatMap((l:any)=>(l.listing_assets??[]).map((x:any)=>{const a:any=assetMap.get(x.asset_id);return a?{id:a.id,listing_id:l.id,title_ar:l.title_ar,price:l.asking_price,area_sqm:a.area_sqm,bedrooms:a.bedrooms,asset_type:a.asset_type,city_id:a.city_id,district_id:a.district_id}:null;})).filter(Boolean);

...
export const GET=withErrorHandling<RouteContext>(async(request,{params})=>{
  ...
  const [interests,viewings,reservationLinks,dealLinks,leaseLinks,ownerships,management,maintenance]=await Promise.all([
  ...
  for(const result of [interests,viewings,reservationLinks,dealLinks,leaseLinks,ownerships,management,maintenance])if('error' in result&&result.error)throw new Error(result.error.message);
  return okResponse({relationships:{interests:interests.data??[],viewings:viewings.data??[],reservations:reservationLinks.data??[],deals:dealLinks.data??[],leases:leaseLinks.data??[],ownerships:ownerships.data??[],management:management.data??[],maintenance:maintenance.data??[]}});
});

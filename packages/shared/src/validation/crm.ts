import { z } from 'zod';
import { DEAL_STATUSES, LISTING_TYPES, PROPERTY_TYPES, VIEWING_OUTCOMES, VIEWING_STATUSES } from '../types/enums';

export const leadRequirementSchema = z.object({
  purpose:z.enum(LISTING_TYPES).optional().nullable(), budget_min:z.number().nonnegative().optional().nullable(), budget_max:z.number().nonnegative().optional().nullable(),
  property_types:z.array(z.enum(PROPERTY_TYPES)).default([]), city_ids:z.array(z.string().uuid()).default([]), district_ids:z.array(z.string().uuid()).default([]),
...
export const viewingInputSchema=z.object({lead_id:z.string().uuid(),property_id:z.string().uuid().optional().nullable(),unit_id:z.string().uuid().optional().nullable(),assigned_user_id:z.string().uuid(),scheduled_at:z.string().datetime(),notes:z.string().optional().nullable()}).refine(v=>Number(!!v.property_id)+Number(!!v.unit_id)===1,{message:'اختر عقارًا أو وحدة واحدة للمعاينة'});

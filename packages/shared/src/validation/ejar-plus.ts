...
export const leaseContractInputSchema=z.object({
 ...
 renewed_from_contract_id:nullableUuid,notes:z.string().optional().nullable(),asset_ids:z.array(uuid).optional().default([]),project_id:nullableUuid,
 parties:z.array(z.object({party_id:uuid,role:z.enum(LEASE_PARTY_ROLES)})).min(2),
}).superRefine((value,context)=>{
 if((value.asset_ids.length===0)===(value.project_id==null))context.addIssue({code:'custom',path:['asset_ids'],message:'اختر مشروعًا كاملًا أو عقارًا/وحدة للعقد'});

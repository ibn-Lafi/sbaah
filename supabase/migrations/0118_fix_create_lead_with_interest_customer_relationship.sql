-- 0118: create_lead_with_interest() still referenced a second legacy column.
-- `customer_relationship` has never existed on the current leads schema.
-- Customer classification is derived from won sale deals and lessee contracts,
-- so it must not be persisted on the lead row.
...
begin
 ...
 insert into leads(tenant_id,full_name,phone,email,source,status,assigned_agent_id,lost_reason)
 values(v_tenant,p_lead->>'full_name',nullif(p_lead->>'phone',''),nullif(p_lead->>'email',''),coalesce(nullif(p_lead->>'source','')::lead_source,'manual'::lead_source),coalesce(nullif(p_lead->>'status','')::lead_status,'new'::lead_status),nullif(p_lead->>'assigned_agent_id','')::uuid,nullif(p_lead->>'lost_reason','')) returning * into v_lead;
 if p_interest is not null then

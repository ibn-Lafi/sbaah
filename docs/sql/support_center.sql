-- مركز دعم سبعة — نفّذ هذا الملف مرة واحدة في Supabase SQL Editor.
...
using (
  ...
  or exists (select 1 from public.platform_admins where auth_user_id = auth.uid())
);
...
with check (exists (select 1 from public.platform_admins where auth_user_id = auth.uid()));
...
using (
  exists (
    ...
    where t.id = ticket_id and (
      t.tenant_id in (select tenant_id from public.users where auth_user_id = auth.uid())
      or exists (select 1 from public.platform_admins where auth_user_id = auth.uid())
    )
  )
);
...
with check (
  sender_type = 'customer' and exists (
    ...
    where t.id = ticket_id
      and t.tenant_id in (select tenant_id from public.users where auth_user_id = auth.uid())
  )
);
...
with check (
  sender_type = 'admin'
  and exists (select 1 from public.platform_admins where auth_user_id = auth.uid())
);
